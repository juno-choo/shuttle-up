    ...
    <div className="container mx-auto py-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-3xl font-bold">
              {isCurrentUserTeam
                ? `${user?.displayName?.split(" ")[0]}'s Team`
                : team.name}
            </h1>
            {isCurrentUserTeam && <Badge variant="default">Your Team</Badge>}
          </div>
        </div>
        {isCurrentUserTeam && <Button variant="outline">Edit Team</Button>}{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <RacketIcon className="h-5 w-5" /> Performance Stats
            </CardTitle>
            <CardDescription>
              Group {team.stats.group} | Rank #{team.stats.rank}
            </CardDescription>
          </CardHeader>
        ...
        <Card className="md:col-span-3">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <CalendarDays className="h-5 w-5" /> Recent Matches
            </CardTitle>
            <CardDescription>
              Last {team.recentMatches.length} matches played.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <ul className="space-y-4">
                {team.recentMatches.map((match: any, index: number) => (
                  <li key={match.id}>
                    ...
                    {index < team.recentMatches.length - 1 && (
